...
class Edit extends Component {
    ...
    editPro = async () => {
        const response = await fetch('/api/edit_promotion', {
          ...
          body: JSON.stringify({ proID : this.state.ID, name: this.state.name, desc: this.state.desc, type: this.state.new_type}),
        });
    }
    editGift = async () => {
        const response = await fetch('/api/edit_gift', {
          ...
          body: JSON.stringify({ proID: this.state.ID, name: this.state.gift_name, type: this.state.gift_type, weight: this.state.weight_name, stock: this.state.gift_stock }),
        });
    }
    editDiscount = async () => {
        const response = await fetch('/api/edit_discount', {
          ...
          body: JSON.stringify({ proID: this.state.ID, itemm: this.state.discount_item, percent: this.state.discount_percent }),
        });
    }
    addDetail = async () => {
    ...
    sendEdit = e => {
        ...
        if(this.state.type !== this.state.new_type){
            ...
            if(this.state.new_type === 'Gift')
                this.editGift();
            else if(this.state.new_type === 'Discount')
                this.editDiscount();
        }
                ...
                <form className="form-container">
                    ...
                    <div className="form-check form-check-inline">
                        <input
                            ...
                            onChange={() => this.setState({ new_type: 'Discount' })}
                        />
